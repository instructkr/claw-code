...
// --- Helper functions used by main.rs ---

fn config_alias_for_current_dir(alias: &str) -> Option<String> {
    let cwd = env::current_dir().ok()?;
    let loader = ConfigLoader::default_for(&cwd);
    let config = loader.load().ok()?;
    config.aliases().get(alias).map(ToOwned::to_owned)
}
...
fn suggest_slash_commands(input: &str) -> Vec<String> {
    ...
    cli::parse::ranked_suggestions(input.trim_start_matches('/'), &candidate_refs)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
/// Starts a minimal MCP server that exposes claw's built-in tools over stdio.
