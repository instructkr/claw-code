...
342. **`claw status --output-format json` and `/status --output-format json` do not include the active session ID in the JSON output, and `workspace.session_id` is always `null` even when resumed with `--resume latest`** — dogfooded 2026-04-30 by Jobdori on `5856913`. Running `claw --output-format json --resume latest /status` and `claw --output-format json status` both return `workspace.session_id: null` and `workspace.session: "live-repl"`. The actual session ID (e.g. `session-1776891003038-0`) is available in `/session list` output but is not echoed back in `/status`, making it impossible for an orchestration script to confirm which session is active from `status` output alone. Additionally, the same `status` output shows `workspace.changed_files: 3` alongside `workspace.staged_files: 0, workspace.unstaged_files: 0, workspace.untracked_files: 3` — if changed_files counts staged+unstaged only (tracked changes), it should be 0, not 3; or if it includes untracked, staged+unstaged+untracked should sum to changed_files. The counter semantics are ambiguous and contradict each other. **Required fix shape:** (a) populate `workspace.session_id` with the actual active session ID string when a session is live or resumed; (b) use a reserved value like `null` only when no session context is available (fresh invocation with no `--resume`); (c) clarify `changed_files` vs `staged_files+unstaged_files+untracked_files` counter semantics and ensure they are mutually consistent or document their independent meanings; (d) add regression coverage proving `claw --resume latest /status --output-format json` populates `workspace.session_id`. Source: Jobdori live dogfood, mengmotaHost, `5856913`, 2026-04-30.
