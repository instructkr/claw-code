...
351. **Top-level `plugins disable <missing-plugin> --output-format json` hangs with zero stdout/stderr instead of returning a typed plugin-not-found response** — dogfooded 2026-04-29 for the 22:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `0f9e8915`. After rebuilding and verifying the binary provenance, repeated bounded runs of `timeout 8 ./rust/target/debug/claw plugins disable does-not-exist --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `plugins list --output-format json` returned promptly with the known plugin inventory payload, proving the plugin command surface is reachable and narrowing the hang to missing-target lifecycle mutation. This is distinct from #350's missing-target `plugins enable` hang: the disable path must also fail safely because it is a separate supported lifecycle verb and may use separate mutation/registry logic. **Required fix shape:** (a) validate the target plugin against discovered/configured inventory before invoking disable-side effects; (b) return bounded stdout JSON such as `kind:"plugin"`, `action:"disable"`, `status:"not_found"` or `kind:"error"`, `code:"plugin_not_found"`, `plugin`, and optional `available_plugins[]`; (c) share the missing-target guard across enable/disable/update/uninstall so destructive lifecycle verbs do not hang silently; (d) add regression coverage proving `plugins disable does-not-exist --output-format json` returns a typed JSON outcome within a deterministic budget and does not mutate plugin state. **Why this matters:** disable is a destructive control-plane operation commonly used during recovery. If a stale plugin name can hang with no bytes, claws cannot tell whether recovery succeeded, the target was absent, or the plugin loader deadlocked, and automated rollback/cleanup flows become unsafe. Source: gaebal-gajae dogfood follow-up for the 22:30 nudge on rebuilt `./rust/target/debug/claw` `0f9e8915`.
