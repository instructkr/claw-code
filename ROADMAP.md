...
346. **Top-level `mcp help --output-format json` can hang with zero stdout/stderr, blocking command-surface discovery without a typed timeout or partial status** — dogfooded 2026-04-29 for the 19:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `b2f7a335`. After rebuilding and verifying `./rust/target/debug/claw version --output-format json` reported `git_sha` `b2f7a335`, running `./rust/target/debug/claw mcp help --output-format json` produced no stdout bytes and no stderr bytes, did not return within the outer probe window, and had to be killed. A direct `timeout 20 ./rust/target/debug/claw mcp help --output-format json` reproduced the same behavior with timeout exit `124`, `stdout=0`, and `stderr=0`. This is distinct from #327's MCP source-list mismatch: here the MCP help JSON surface fails to produce any response at all, so command-surface discovery stalls before the caller can even inspect whether the source list is accurate. **Required fix shape:** (a) make `mcp help --output-format json` return promptly with a bounded, non-networked help payload; (b) if MCP help needs runtime discovery, emit structured lifecycle events or a typed JSON error before any long-running probe; (c) add a CLI-level timeout or internal guard so JSON help commands cannot hang silently with zero output; (d) add regression coverage proving `mcp help --output-format json` completes within a short deterministic budget and emits valid stdout JSON even when MCP transports/config discovery are slow or unavailable. **Why this matters:** help/discovery commands are supposed to be the safe preflight path. If `mcp help --output-format json` can hang silently, claws cannot distinguish slow MCP transport, deadlock, config probe, or CLI bug, and the rest of the dogfood cycle loses a reliable way to enumerate MCP lifecycle surfaces. Source: gaebal-gajae dogfood follow-up for the 19:30 nudge on rebuilt `./rust/target/debug/claw` `b2f7a335`.
